...
import { useSelect, useGetIdentity, useNavigation, useParsed } from "@refinedev/core";
...
export const CotizacionEdit = () => {
      ...
      const fetchData = async () => {
        try {
          ...
          reset({
            ...
            origen: cotizacion.origen || "Local",
  ...
  const onFinish = async (data: FormValues) => {
    ...
    try {
      ...
      // 1. Actualizar Cotización (Master)
      ...
      // 3. Preparar e Insertar Nuevos Items
        ...
        if (emailTo) {
          ...
          await fetch(apiUrl, {
            ...
            body: JSON.stringify({
              ...
              quoteId: String(id).split('-')[0].toUpperCase(),
              items: itemsForEmail,
              subtotal,
              iva,
              total,
          ...
          toast.success("Cotización actualizada y correo enviado");
        } else {
          toast.success("Cotización actualizada (Cliente sin correo)");
        }
      } catch (emailErr) {
        console.error("Error enviando correo:", emailErr);
        toast.warning("Cotización actualizada, pero hubo un error enviando el correo");
      }
    ...
    <form onSubmit={handleSubmit(onFinish)} className="flex flex-col gap-6 max-w-6xl mx-auto pb-10">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Modificar cotización multi-línea existente</p>
        </div>
      </div>
      ...
      <Card>
        ...
        <CardFooter className="flex justify-end gap-2">
          ...
          <Button type="submit" disabled={isSubmitting}>
            {isSubmitting ? "Guardando..." : "Actualizar Cotización"}
